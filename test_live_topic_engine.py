...
def run_live_test(region: str = "all"):
    ...
    if not live_feeds:
        print("⚠️ No live feeds returned (network offline or feeds blocked). Using fallback feed items.")
    ...
    if selected:
        ...
        print(f"⚠️ Saturation    : {selected.sat_score:.2f} (Competition Penalty)")
        print("=" * 80)
    else:
