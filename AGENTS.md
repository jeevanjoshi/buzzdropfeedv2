...
5. **Active Thread Reply Bot (`ActiveThreadSeeder`):** Interacts globally across Reddit to inject helpful comments into highly active discussions from the last 24 hours. The bot reads thread context, calls the LLM for custom context-aware replies containing verified facts, and naturally appends a YouTube citation at the end. Backend chain (first available wins): PRAW (`REDDIT_CLIENT_ID` set) → Playwright browser poster → read-only public `.json` client. Supports a soft warm-up toggle (`ACTIVE_SEEDER_WARMUP=1`) to comment without posting links.
6. **Browser-driven Reddit engagement (`RedditBrowserPoster`, `src/engine/reddit_browser_poster.py`):** Fully-automated comment poster for the Pi via Playwright Chromium that works where `.json`/PRAW are IP-blocked. Uses a real Chrome-derived binary (`REDDIT_CHROMIUM_PATH`, default `/usr/bin/chromium`), persists login sessions to `logs/reddit_sessions/`, rotates the gitignored account pool `reddit_accounts.json` (per-account `daily_cap`), serialises chromium launches behind RAM + process-count resource guards so bursts can't OOM the Pi's 4GB, posts via the old.reddit `<textarea name=text>` form, best-effort logged-out visibility verification, learns per-subreddit permissiveness, and retires accounts after sustained unverified (shadowban) streaks (`REDDIT_RETIRE_AFTER_UNVERIFIED`, default 3). State persists to `logs/reddit_rotation_state.json`. `RedditJsonClient` (`reddit_json_client.py`) is a throttled, OAuth-free fallback for thread discovery/comment context only.
7. **Pi edge warm-up on publish:** after a real upload, `publisher.py` triggers `reddit_warmup.py` on the residential Pi via a **fire-and-forget SSH** call (`_trigger_pi_warmup`) — low-volume, NO-LINK informative comments drawn from per-domain banks posted to permissive subreddits to build account trust so later link comments stick. Env: `REDDIT_PI_WARMUP_ON_PUBLISH` (default `1`; set `0` to disable), `REDDIT_WARMUP_COUNT` (default 3), Pi connection via `PI5_IP`/`PI5_USER`/`PI5_TARGET_DIR` (mirrors `sync_to_pi.sh` defaults). Non-fatal — a failure never affects the publish result.
8. **Generic link seeder (`reddit_link_seeder.py`):** reusable, data-driven campaign seeder that reads ALL published videos from `logs/state_*.json` (minus `exclude_video_ids` in `seed_campaigns.json`), discovers active threads per topic, dynamically matches each thread to the **most relevant** published video (keyword overlap + learned sub permissiveness + sub size), and posts genuine on-topic comments with that video's YouTube link into niche-but-active subreddits (cap `REDDIT_LINK_MAX_PER_RUN`, default 1; `--video <id>` / `--max N`; skips subs > `niche_max_subscribers`, default 150k). `post_reddit_links.py` is a one-off ops variant with hardcoded videos, and `cleanup_pi.py` prunes old logs/media/state dirs on the Pi (`CSVG_KEEP_MEDIA_RUNS`, `CSVG_KEEP_STATE_FILES`) to keep disk bounded.

## Fail-fast TTS & subtitle integrity (Pi outage == abort, never fake media)
When the Pi edge (`AUDIO_EDGE_URL`) is unreachable **and** no real neural TTS is available locally, the run **aborts immediately on the first shot** instead of silently shipping placeholder audio. Placeholder/timer output is a hard kill in `media_producer.py`:
- **Toner wav** (`engine == "synthetic_wav_fallback"` from either the remote edge response or the local `synthesize_tts` fallback) → `RuntimeError`. This tone (1%-amplitude repeating beep) is NOT speech and used to pass Gate 2 (wav >1KB) silently; now it fails fast so the checkpoint stays resume-safe.
- **Whisper-degraded subtitles** — a single dummy `NARRATION` `Dialogue:` line (written when Whisper alignment fails with no real word timestamps) → Gate 3 early `RuntimeError`; also re-checks on resume.
- Resume safety: the run keeps its `SCRIPT_APPROVED` checkpoint, so once the Pi is back you can `--resume` the same pipeline_id to regenerate media; only a *real* local Kokoro (`kokoro_onnx`) fallback is ever allowed to continue offline.
