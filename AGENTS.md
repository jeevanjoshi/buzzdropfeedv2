...
- Nano-banana thumbnail env: `CSVG_NANO_BANANA_THUMBNAILS=1` (default on) + `GEMINI_API_KEY`; images route through Vertex (`GOOGLE_CLOUD_PROJECT`) when set else the AI-Studio key. OAuth caption transcripts (`fetch_caption_transcript_oauth`) need a token containing `youtube.force-ssl`.
